...
void moolticute_cb_param_changed(struct json_object *jObj)
{
  ...
  {
    mContext.info.parameters.delay_after_key_enabled=isTrue(value);
  }
  else if(strncmp(param, "key_after_pass", 14) == 0)
  {
    mContext.info.parameters.key_after_pass=strtol(value,NULL,10);
  }
  else if(strncmp(param, "key_after_pass_enabled", 22) == 0)
  {
    mContext.info.parameters.key_after_pass_enabled=isTrue(value);
  }
  else if(strncmp(param, "key_after_login", 15) == 0)
  {
    mContext.info.parameters.key_after_login=strtol(value,NULL,10);
  }
  else if(strncmp(param, "key_after_login_enabled", 23) == 0)
  {
    mContext.info.parameters.key_after_login_enabled=isTrue(value);
  }
  else if(strncmp(param, "lock_unlock_mode", 16) == 0)
  {
    mContext.info.parameters.lock_unlock_mode=strtol(value,NULL,10);
  }
  else if(strncmp(param, "hash_display", 12) == 0)
  {
    mContext.info.parameters.hash_display=isTrue(value);
  }
  else if(strncmp(param, "random_starting_pin", 19) == 0)
  {
    mContext.info.parameters.random_starting_pin=isTrue(value);
  }
  else if(strncmp(param, "knock_enabled", 13) == 0)
  {
    mContext.info.parameters.knock_enabled=isTrue(value);
  }
  else if(strncmp(param, "knock_sensitivity", 17) == 0)
  {
    mContext.info.parameters.knock_sensitivity=strtol(value,NULL,10);
  }
  else if(strncmp(param, "screen_brightness", 17) == 0)
  {
    mContext.info.parameters.screen_brightness=strtol(value,NULL,10);
  }
  else if(strncmp(param, "user_interaction_timeout", 24) == 0)
  {
    mContext.info.parameters.user_interaction_timeout=strtol(value,NULL,10);
  }
  else if(strncmp(param, "tutorial_enabled", 16) == 0)
  {
    mContext.info.parameters.tutorial_enabled=isTrue(value);
  }
  else if(strncmp(param, "offline_mode", 12) == 0)
  {
    mContext.info.parameters.offline_mode=isTrue(value);
  }
  else if(strncmp(param, "flash_screen", 12) == 0)
  {
    mContext.info.parameters.flash_screen=isTrue(value);
  }
  else if(strncmp(param, "user_request_cancel", 19) == 0)
  {
    mContext.info.parameters.user_request_cancel=isTrue(value);
  }
  else if(strncmp(param, "screensaver", 11) == 0)
  {
    mContext.info.parameters.screensaver_enabled=isTrue(value);
  }
  else if(strncmp(param, "lock_timeout_enabled", 20) == 0)
  {
    mContext.info.parameters.lock_timeout_enabled=isTrue(value);
  }
  else if(strncmp(param, "lock_timeout", 12) == 0)
  {
    mContext.info.parameters.lock_timeout=strtol(value,NULL,10);
  }
  else if(strncmp(param, "keyboard_layout", 15) == 0)
  {
    mContext.info.parameters.keyboard_layout=strtol(value,NULL,10);
  }
  else
  {
